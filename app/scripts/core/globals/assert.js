...
(function () {
    ...
    // This usually occurs when running tests and do not use mue.util.isBoolean at this point as it may not be loaded yet
    if (typeof mue.ENABLE_ASSERTS !== 'boolean') {
        mue.ENABLE_ASSERTS = true;
    }
    ...
    var handleFailure = function (defaultMessage, defaultArgs, givenMessage, givenArgs) {
        ...
        var error = new Error(mue.string.subs.apply(null, messageArgs));

        // Call all registered handlers but make sure they can prevent the default handler
        mue.util.forEach(errorHandlers, function (handler) {
            try {
                if (mue.util.isFunction(handler)) {
                    handler(error);
                }
            } catch (e) {
            }
    ...
    mue.assert = {
        ...
        addErrorHandler: function (errorHandlerFn) {
            if (mue.ENABLE_ASSERTS && mue.util.isFunction(errorHandlerFn)) {
        ...
        /**
         * Checks if the condition evaluates to true if mue.ENABLE_ASSERTS is true.
        ...
        assert: function (condition, opt_message, var_args) {
            if (mue.ENABLE_ASSERTS && !condition) {
        ...
        /**
         * Checks if the value is a number if mue.ENABLE_ASSERTS is true.
        ...
        assertNumber: function (value, opt_message, var_args) {
            if (mue.ENABLE_ASSERTS && !mue.util.isNumber(value)) {
                handleFailure('Expected number but got %s: %s.',
                    [mue.util.typeOf(value), value], opt_message,
        ...
        assertDefinedAndNotNull: function (value, opt_message, var_args) {
            if (mue.ENABLE_ASSERTS && !mue.util.isDefinedAndNotNull(value)) {
                handleFailure('Expected a defined value but got %s.',
                    [mue.util.typeOf(value)], opt_message,
                    Array.prototype.slice.call(arguments, 2));
            }
        },
        ...
        assertHasLength: function (value, opt_message, var_args) {
            if (mue.ENABLE_ASSERTS) {
                mue.assert.assertNumber(value.length, 'Expected a length property');
        ...
        /**
         * Checks if the value is a string if mue.ENABLE_ASSERTS is true.
         * @param {*} value The value to check.
         * @param {string=} opt_message Error message in case of failure.
         * @param {...*} var_args The items to substitute into the failure message.
        ...
        assertString: function (value, opt_message, var_args) {
            if (mue.ENABLE_ASSERTS && !mue.util.isString(value)) {
        ...
        /**
         * Checks if the value is a string if mue.ENABLE_ASSERTS is true and that it is length.
         * @param {*} value The value to check.
         * @param {string=} opt_message Error message in case of failure.
         * @param {...*} var_args The items to substitute into the failure message.
         * @throws {AssertionError} When the value is not a string.
        ...
        assertStringWithLength: function (value, opt_message, var_args) {
            if (mue.ENABLE_ASSERTS) {
                mue.assert.assertString(value, opt_message, var_args);
                mue.assert.assertHasLength(value, opt_message, var_args);
            }
        },
        ...
        assertArrayWithLength: function (value, opt_message, var_args) {
            mue.assert.assertArray(value, opt_message, var_args);
            mue.assert.assertHasLength(value, opt_message, var_args);
        },

        /**
         * Checks if the value is a function if mue.ENABLE_ASSERTS is true.
         * @param {*} value The value to check.
         * @param {string=} opt_message Error message in case of failure.
         * @param {...*} var_args The items to substitute into the failure message.
        ...
        assertFunction: function (value, opt_message, var_args) {
            if (mue.ENABLE_ASSERTS && !mue.util.isFunction(value)) {
        ...
        /**
         * Checks if the value is an Object if mue.ENABLE_ASSERTS is true.
         * @param {*} value The value to check.
         * @param {string=} opt_message Error message in case of failure.
         * @param {...*} var_args The items to substitute into the failure message.
         * @throws {AssertionError} When the value is not an object.
        ...
        assertObject: function (value, opt_message, var_args) {
            if (mue.ENABLE_ASSERTS && !mue.util.isObject(value)) {
                handleFailure('Expected object but got %s: %s.',
                    [mue.util.typeOf(value), value],
                    opt_message, Array.prototype.slice.call(arguments, 2));
            }
        },

        /**
         * Checks if the value is an Array if mue.ENABLE_ASSERTS is true.
         * @param {*} value The value to check.
         * @param {string=} opt_message Error message in case of failure.
         * @param {...*} var_args The items to substitute into the failure message.
        ...
        assertArray: function (value, opt_message, var_args) {
            if (mue.ENABLE_ASSERTS && !mue.util.isArray(value)) {
        ...
        /**
         * Checks if the value is a boolean if mue.ENABLE_ASSERTS is true.
         * @param {*} value The value to check.
         * @param {string=} opt_message Error message in case of failure.
         * @param {...*} var_args The items to substitute into the failure message.
        ...
        assertBoolean: function (value, opt_message, var_args) {
            if (mue.ENABLE_ASSERTS && !mue.util.isBoolean(value)) {
        ...
        /**
         * Checks if the value is a DOM Element if mue.ENABLE_ASSERTS is true.
         * @param {*} value The value to check.
         * @param {string=} opt_message Error message in case of failure.
         * @param {...*} var_args The items to substitute into the failure message.
         * @throws {AssertionError} When the value is not a boolean.
        ...
        assertElement: function (value, opt_message, var_args) {
            if (mue.ENABLE_ASSERTS && (!mue.util.isObject(value) ||
                value.nodeType != mue.util.dom.NodeType.ELEMENT)) {
        ...
        /**
         * Checks if the value is an instance of the user-defined type if mue.ENABLE_ASSERTS is true.
         *
        ...
        assertInstanceof: function (value, type, opt_message, var_args) {
            if (mue.ENABLE_ASSERTS && !(value instanceof type)) {
        ...
        /**
         * Checks if the value is a angular $scope if mue.ENABLE_ASSERTS is true.
         * @param {*} value The value to check.
         * @param {string=} opt_message Error message in case of failure.
         * @param {...*} var_args The items to substitute into the failure message.
        ...
        assertScope: function (value, opt_message, var_args) {
            if (mue.ENABLE_ASSERTS && !mue.util.isScope(value)) {
        ...
        assertNullOrUndefined: function (value, opt_message, var_args) {
            if (mue.ENABLE_ASSERTS && !mue.util.isNullOrUndefined(value)) {
